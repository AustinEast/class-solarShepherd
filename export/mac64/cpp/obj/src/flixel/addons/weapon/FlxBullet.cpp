...
Void FlxBullet_obj::fireAtPosition( Float FromX,Float FromY,Float ToX,Float ToY,int Speed){
{
		...
		if ((this->accelerates)){
			...
			{
				HX_STACK_LINE(133)
				::flixel::util::FlxPoint point = ::flixel::util::FlxPoint_obj::_pool->get()->__Field(HX_CSTRING("set"),true)(ToX,ToY);		HX_STACK_VAR(point,"point");
				HX_STACK_LINE(133)
				point->_inPool = false;
				HX_STACK_LINE(133)
				_g4 = point;
...
Void FlxBullet_obj::fireFromAngle( Float FromX,Float FromY,int FireAngle,int Speed){
{
		...
		HX_STACK_LINE(165)
		{
			...
			HX_STACK_LINE(165)
			Float Angle = (FireAngle + _g4);		HX_STACK_VAR(Angle,"Angle");
			HX_STACK_LINE(165)
			int _g5 = ::flixel::util::FlxRandom_obj::intRanged(-(this->_weapon->rndFactorSpeed),this->_weapon->rndFactorSpeed,null());		HX_STACK_VAR(_g5,"_g5");
			HX_STACK_LINE(165)
			Float Speed1 = (Speed + _g5);		HX_STACK_VAR(Speed1,"Speed1");
			HX_STACK_LINE(165)
			...
			HX_STACK_LINE(165)
			Float _g9 = (_g8 * Speed1);		HX_STACK_VAR(_g9,"_g9");
			HX_STACK_LINE(165)
			result->set_y(_g9);
			HX_STACK_LINE(165)
			newVelocity = result;
