...
HX_STACK_ARG(data,"data")
{
	...
	if ((::Std_obj::is(data,hx::ClassOf< ::haxe::xml::Fast >()))){
		...
		HX_STACK_LINE(42)
		if ((::Std_obj::is(data,hx::ClassOf< ::openfl::_legacy::utils::ByteArray >()))){
			HX_STACK_LINE(44)
...
::openfl::_legacy::geom::Rectangle TiledTileSet_obj::getRect( int ID){
	...
	HX_STACK_ARG(ID,"ID")
	HX_STACK_LINE(154)
	return ::openfl::_legacy::geom::Rectangle_obj::__new((hx::Mod(ID,this->numCols) * this->tileWidth),((Float(ID) / Float(this->numCols)) * this->tileHeight),null(),null());
}
